...
func findSafeguardStressWasm(t *testing.T) (wasmPath, manifestPath string) {
	...
	wasmPath = filepath.Join(repoRoot, "examples", "safeguard-stress", "safeguard-stress.wasm")
	manifestPath = filepath.Join(repoRoot, "examples", "safeguard-stress", "plugin.manifest.json")
	if _, err := os.Stat(wasmPath); err != nil {
		t.Skipf("safeguard-stress wasm not built; run tools/build-plugin.sh examples/safeguard-stress")
	}
