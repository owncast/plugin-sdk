...
import (
	...
	"github.com/owncast/owncast-plugins/owncast/kv"
)
...
const (
	ChatSendSystem ChatSendKind = iota // owncast.chat.send
	ChatSendAction                     // owncast.chat.sendAction (italic-style)
)
...
func hostSendChat(sink func(ChatSendRequest), pluginName string) extism.HostFunction {
	fn := extism.NewHostFunctionWithStack(
		...
		func(ctx context.Context, p *extism.CurrentPlugin, stack []uint64) {
			...
			if sink != nil {
				sink(ChatSendRequest{PluginName: pluginName, Kind: ChatSendSystem, Text: text})
			}
		},
		[]extism.ValueType{extism.ValueTypePTR},
		[]extism.ValueType{},
