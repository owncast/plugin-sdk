...
| Compose with other plugins                      | emit `owncast.events.emit`, receive `on:{}`  | `owncast.events.emit(suffix, payload)`, delivered as `<your-slug>.<suffix>` | `events.emit` (emitter only)           |
