...
**Demonstrates:** the permission split between read-only telemetry and settable config — `owncast.stream.broadcaster()` under `server.read` (there's nothing to write), the `owncast.video_config.read()` reader under `videoconfig.read`, and the partial `owncast.video_config.write({...})` mutation under the separate, higher-privilege `videoconfig.write`.
