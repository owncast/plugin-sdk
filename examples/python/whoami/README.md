...
a known chat user) or missing (`None`, when they haven't joined chat). The
...
def me(req):
    if not req.user:
        return {"status": 401, ...}
    return {"status": 200, "body": json.dumps({"identified": True, "user": req.user.raw})}
```
...
```bash
owncast-plugin-py test  examples/python/whoami     # build and run the tests
owncast-plugin-py serve examples/python/whoami     # build and serve at http://localhost:8080/plugins/whoami/
```
