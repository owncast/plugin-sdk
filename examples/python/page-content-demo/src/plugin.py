...
#
# The JS version uses the `mustache` npm package. Python can't use it, so
# this file ships a minimal Mustache-subset renderer that supports the
# features the two templates use: {{var}} (HTML-escaped), {{{var}}}/{{&var}}
# (unescaped), dotted paths, {{#section}}/{{/section}} (truthy / list
# iteration), {{^inverted}}/{{/inverted}}, and {{.}} (current item). It
# produces the same HTML the JS Mustache.render() produces.
import re
...
def banner(req):
    ...
    return _render(_tpl("greeting.mustache"), {"displayName": display_name})
