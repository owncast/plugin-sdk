...
base64-encoded, and the plugin carries a tiny dependency-free base64 codec since
the wasm runtime doesn't guarantee `atob`/`btoa`.

Unlike `storage.upload`, these files stay server-side and are never served
