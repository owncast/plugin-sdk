...
**Demonstrates:** `http.serve`, static-asset serving from `public/`, dynamic HTTP handlers via `onHttpRequest`, `owncast.chat.history(limit)`, realtime `owncast.sse.send(...)` from `onChatMessage`, and the `http.sse` permission.
